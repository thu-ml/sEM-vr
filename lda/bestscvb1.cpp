...
BestSCVB1::BestSCVB1(Corpus &corpus, Corpus &to_corpus, Corpus &th_corpus,
...
void BestSCVB1::Theta(double *cdk, double *theta, int N) {
    double Z = 1.0 / (N + alpha_bar);
    for (int k = 0; k < K; k++)
        theta[k] = (cdk[k] + alpha) * Z;
}

void BestSCVB1::Phi(std::vector<double> &cvk)
{
...
#pragma omp parallel for
            for (int id = bs; id < be; id++) {
                ...
                for (auto v: corpus.w[d]) {
                    ...
                    sum = corpus.w[d].size() / sum;
                    double scale = 1.0 / (corpus.w[d].size() + alpha_bar);
                    for (int k = 0; k < K; k++) {
                        cd[k] += step_size * (prob[k] * sum - cd[k]);
...
double BestSCVB1::BatchLogLikelihood()
{
...
double BestSCVB1::BatchPerplexity()
{
