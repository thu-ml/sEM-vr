...
ICVBOEM::ICVBOEM(Corpus &corpus, Corpus &to_corpus, Corpus &th_corpus,
...
void ICVBOEM::Phi(std::vector<double> &cvk)
{
...
void ICVBOEM::InferenceDocument(int d, double *th, double *phi, double *doc_buf, double *var_in, double *var_out) {
    ...
    for (int iiter = 0; iiter < FLAGS_num_inf_iters; iiter++) {
        ...
        for (int k = 0; k < K; k++)
            var[k] = next_cd[k] * exp(-next_var[k] / (2 * sqr(next_cd[k])));
    }
    if (th != nullptr)
        for (int k = 0; k < K; k++)
                th[k] = next_cd[k] * scale;
    if (var_out != nullptr)
        copy(var, var+K, var_out);
}

void ICVBOEM::Estimate()
{
...
double ICVBOEM::BatchLogLikelihood()
{
...
double ICVBOEM::BatchPerplexity()
{
